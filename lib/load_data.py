...
def get_data(ids, path, im_height, im_width, train, feature=False):
    im_channels = 4 if feature else 3
    #ids = sorted(next(os.walk(path + "/images"))[2])
    X = np.zeros((len(ids), im_height, im_width, im_channels), dtype=np.float32)
    print(X.shape)
    ...
    for n, id_ in tqdm(enumerate(ids), total=len(ids)):
        ...
        x_img = x_img.squeeze() / 255.0
        # Load masks
        if train:
            name, ext = os.path.splitext(id_) # using splitext to handle "." in the filename
            # mask = cv2.imread(path + '/masks/' + '{}'.format(name + '_mask' + ".png"), cv2.IMREAD_GRAYSCALE)

            # mask = cv2.imread(path + '/masks/' + '{}'.format(id_.split('.')[0]+ '_mask'+ '.png'), cv2.IMREAD_GRAYSCALE)

            # mask = cv2.imread(path + '/masks/' + '{}'.format(id_.split('.')[0]+ '.png'), cv2.IMREAD_GRAYSCALE)

            mask = cv2.imread(path + '/masks/' + '{}'.format('mask_' + id_.split('.')[0].split('_')[-1] + '.png'), cv2.IMREAD_GRAYSCALE)

            mask = img_to_array(mask)
            mask = resize(mask, (im_height, im_width, 1), mode='constant', preserve_range=True)

        if feature:
            canny_feature = cv2.imread(path + '/features_blurred/' + '{}'.format('feature_' + id_.split('.')[0] + '.png'), cv2.IMREAD_GRAYSCALE)

            canny_feature = img_to_array(canny_feature)
            canny_feature = resize(canny_feature, (im_height, im_width, 1), mode='constant', preserve_range=True) / 255.0
            
            x_img = np.dstack((x_img, canny_feature))

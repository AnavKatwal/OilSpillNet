from tensorflow.keras.layers import Input, Conv2D, Conv2DTranspose, DepthwiseConv2D, Dense, MaxPooling2D, GlobalAveragePooling2D, AveragePooling2D, BatchNormalization, Activation, concatenate, Reshape, multiply, add, Permute,GlobalMaxPooling2D
...
def IterLUNet_V2(input_filters, height, width, n_channels):
    ...
    # Iteration 1
    block1 = initial_conv2d_bn(inputs, filters*1, 3, 3, padding='same', activation='gelu')
    block1 = initial_conv2d_bn(block1, filters*1, 3, 3, padding='same', activation='gelu')
    #pool1 = MaxPooling2D(pool_size=(2, 2))(block1)

    bottleneck1 = attention_block(block1, filters*2, name = 'iterLBlock1')


    up1 = concatenate([bottleneck1, block1], axis=3)
    level1 = iterLBlock(up1, filters*1, name = 'iterLBlock2' )


    # Iteration 2

    encoder2 = initial_conv2d_bn(inputs, filters*1, 3, 3, padding='same', activation='gelu')
    encoder = initial_conv2d_bn(encoder2, filters*1, 3, 3, padding='same', activation='gelu')
    ...
    inter21 = concatenate([block2, bottleneck1], axis=3)
    ...
    bottleneck2 = attention_block(block21, filters*8, name = 'iterLBlock5')

    up21 = concatenate([bottleneck2, block21], axis=3)
    block22 = iterLBlock(up21, filters*4, name = 'iterLBlock6')


    up22 = concatenate([block22, block2], axis=3)

    level2 = iterLBlock(up22, filters*2, name = 'iterLBlock7')

    conv10 = Conv2D(1, (1, 1), padding="same", activation="sigmoid", name='visualized_layer')(level2)
...
# Define the model
    ...
    model = IterLUNet_V2(64, 512, 512, 3)
    print(model.summary())

...
def TLoopNet_Notebook(input_filters, height, width, n_channels):
    ...
    for i, layer in enumerate(base_model.layers[:-38]):
        layer.trainable = False

    # Iteration 1
    s1 = base_model.get_layer("input_1").output                ## (256 x 256)
    s2 = base_model.get_layer("block_1_expand_relu").output    ## (128 x 128)
    s3 = base_model.get_layer("block_3_expand_relu").output    ## (64 x 64)

    """ Bridge """
    b1 = channel_spatial_squeeze_excite(base_model.get_layer("block_5_expand_relu").output)   ## (32 x 32)
    b1 = iterLBlock(b1, filters*8)
    """ Decoder """
    ...
    s21 = base_model.get_layer("block_1_expand_relu").output    ## (128 x 128)
    s21 = concat_feature_rank(s21, d2, filters*2)
    s31 = base_model.get_layer("block_3_expand_relu").output    ## (64 x 64)
    s31 = concat_feature_rank(s31, d1, filters*4)
    s41 = base_model.get_layer("block_6_expand_relu").output    ## (32 x 32)
    s41 = concat_feature_rank(s41, b1, filters*8)

    """ Bridge """
    b11 = channel_spatial_squeeze_excite(base_model.get_layer("block_13_expand_relu").output)   ## (16 x 16)
    ...
    model = Model(model_input, outputs, name="TLoopNet_Notebook")
...
# Define the model
    ...
    model = TLoopNet_Notebook(16, 384, 384, 3)

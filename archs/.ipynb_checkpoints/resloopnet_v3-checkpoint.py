...
def concat_feature_rank(encoder, decoder, filters):
    feature_vector = iterLBlock(encoder, filters)
...
def ResLoopNet_V3(input_filters, height, width, n_channels):
    ...
    base_model = tf.keras.applications.ResNet152V2(weights="imagenet", include_top=False, input_tensor=model_input, pooling=max)
    ...
    model = Model(model_input, outputs, name="ResLoopNet_V3")
...
# Define the model
    ...
    model = ResLoopNet_V3(16, 256, 256, 3)

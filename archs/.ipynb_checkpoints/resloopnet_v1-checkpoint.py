...
def iterLBlock(x, filters, name=None):
    ...
    #pool_proj_2 = AveragePooling2D((3, 3), strides=(1, 1), padding='same')(x)
    #pool_proj = add([pool_proj_1, pool_proj_2])
    ...
    #output = add([shortcut,concat_output])
...
def ResLoopNet_V1(input_filters, height, width, n_channels):
    ...
    model = Model(model_input, outputs, name="ResLoopNet_V1")
...
# Define the model
    ...
    model = ResLoopNet_V1(32, 384, 384, 3)
    #mnet = MobileNetV2(input_tensor=inputs, input_shape = (384, 384, 3), include_top=False, weights="imagenet", alpha=1)
